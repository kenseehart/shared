...
logger = logging.getLogger("ken-mcp.personal-auth")

DEFAULT_ACCESS_TOKEN_EXPIRY = 30 * 24 * 60 * 60
DEFAULT_STATE_DIR = ".oauth-state"


class PersonalAuthProvider(InMemoryOAuthProvider):
    """OAuth 2.1 provider with static client ID + secret for Claude.ai connectors."""
    ...
    def _load_state(self) -> None:
        ...
        try:
            ...
            logger.warning("Failed to load OAuth state: %s", exc)
    ...
    def _validate_client(self, client: OAuthClientInformationFull) -> None:
        if self._static_client_id and self._static_client_secret:
            if client.client_id != self._static_client_id:
